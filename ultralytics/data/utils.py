...
def verify_image_label(args):
    ...
    try:
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file) as f, open(lb_rgb) as f_rgb:
                ...
                if len(lb_rgb) == 0:
                    ...
                    cls_rgb = np.array([-1])
                    boxes_rgb = np.array([0,0,0,0])
                else:
                    center_point = np.array([lb_rgb[0][1], lb_rgb[0][2]])
                    cls_rgb = np.array([lb_rgb[0][0]])
                    boxes_rgb = np.array(lb_rgb[0][1:])
                
                if len(lb) == 0:
                    boxes_ir = np.array([0,0,0,0])
                else:
                    boxes_ir = np.array(lb[0][1:])
        ...
        lb = lb[:, :5]
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg, offset, area, center_point, cls_rgb, boxes_rgb, boxes_ir
    except Exception as e:
        nc = 1
        msg = f"{prefix}WARNING ⚠️ {im_file}: ignoring corrupt image/label: {e}"
        return [None, None, None, None, None, nm, nf, ne, nc, msg, offset, area, center_point, cls_rgb, boxes_rgb, boxes_ir]
