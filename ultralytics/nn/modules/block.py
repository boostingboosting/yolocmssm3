...
class C2PSA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, e=0.5):
        ...
        self.m = nn.Sequential(*(PSABlock(self.c, attn_ratio=0.5, num_heads=self.c // 16) for _ in range(n)))
...
def get_activation(act: str, inpace: bool = False):
    '''get activation
    '''
    act = act.lower()
...
class MessageAgg(nn.Module):
    def __init__(self, agg_method="weighted", eps=1e-8):
        super().__init__()
    ...
    def forward(self, X, path):
        """
        X:    [B, S, C]
        path: [B, T, S]  (可为0/1，也可为非负权重)
        """
        if self.agg_method == "sum":
            return torch.matmul(path, X)  # [B,T,C]

        if self.agg_method == "mean":
            X_sum = torch.matmul(path, X)
            denom = torch.sum(path, dim=2, keepdim=True).clamp_min(1.0)
            out = X_sum / denom
            out = torch.where(torch.sum(path, dim=2, keepdim=True) > 0, out, torch.zeros_like(out))
            return out

        if self.agg_method == "weighted":
            # =====================================================
            # ### 修改：带权行归一化（2N 统一归一化）
            # alpha_{t,s} = path_{t,s} / sum_s path_{t,s}
            # out_t = Σ_s alpha_{t,s} X_s
            # =====================================================
            denom = torch.sum(path, dim=2, keepdim=True)                 # [B,T,1]
            alpha = path / (denom + self.eps)                            # [B,T,S]
            out = torch.matmul(alpha, X)                                 # [B,T,C]
            out = torch.where(denom > 0, out, torch.zeros_like(out))     # 无邻居置0
            return out

        # fallback
        return torch.matmul(path, X)


class HyPConv(nn.Module):
    def __init__(self, c1, c2, agg="weighted"):
        super().__init__()
        self.fc_ir = nn.Linear(c1, c2)
        self.fc_rgb = nn.Linear(c1, c2)
        self.v2e = MessageAgg(agg_method=agg)
        self.e2v = MessageAgg(agg_method=agg)

    def forward(self, x_ir, x_rgb, H):
        """
        x_ir : [B, N, c1]
        x_rgb: [B, N, c1]
        H    : [B, 2N, 2N]  带权邻接/关联矩阵（非负）
        """
        # ### 修改：各自 fc，再拼接
        x_ir = self.fc_ir(x_ir)            # [B,N,c2]
        x_rgb = self.fc_rgb(x_rgb)         # [B,N,c2]
        x = torch.cat([x_ir, x_rgb], dim=1)  # [B,2N,c2]
...
class HyperComputeModule(nn.Module):
    ...
    你最终简化版：
      ...
      - 不用 lambda
      - W_ir_ir = S_ir
      - W_ir_rgb = S_cross * M_rgb
      - W_rgb_ir = S_cross^T * M_ir
      - W_rgb_rgb = S_rgb
    """
    def __init__(
        self,
        c1, c2,
        thr_ir,
        sigma_ir=None, sigma_rgb=None, sigma_cross=None,
        residual=True
    ):
        super().__init__()
        self.thr_ir = float(thr_ir)
        self.thr_rgb = float(thr_ir)

        # ### sigma 默认 thr/2
        self.sigma_ir = float(sigma_ir) if sigma_ir is not None else self.thr_ir / 2.0
        self.sigma_rgb = float(sigma_rgb) if sigma_rgb is not None else self.thr_rgb / 2.0

        # ### cross 的 sigma：你可以先直接用 max(thr_ir, thr_rgb)/2，避免引入 thr_cross
        base_thr = max(self.thr_ir, self.thr_rgb)
        self.sigma_cross = float(sigma_cross) if sigma_cross is not None else base_thr / 2.0

        self.residual = bool(residual)

        self.hgconv = HyPConv(c1, c2, agg="weighted")

        # ### 残差通道不一致投影
        self.res_proj = None
        if c1 != c2:
            self.res_proj = nn.Linear(c1, c2, bias=False)

        self.fuse = nn.Conv2d(2 * c2, c2, kernel_size=1, bias=False)
    ...
    def forward(self,x):
        """
        x_ir_map : [B, C1, H, W]
        x_rgb_map: [B, C1, H, W]   (建议输入已对齐RGB)
        return:
          z:   [B, C2, H, W]
          H:   [B, 2N, 2N]  (调试可视化用)
        """
        x_ir_map, x_rgb_map = x[0],x[1]
        # print(x_ir_map.shape)
        B, C1, H, W = x_ir_map.shape
        N = H * W

        # 1) flatten
        ir_node = x_ir_map.view(B, C1, N).transpose(1, 2).contiguous()    # [B,N,C1]
        rgb_node = x_rgb_map.view(B, C1, N).transpose(1, 2).contiguous()  # [B,N,C1]

        # 2) 距离
        D_ir = torch.cdist(ir_node, ir_node)        # [B,N,N]
        D_rgb = torch.cdist(rgb_node, rgb_node)     # [B,N,N]
        D_cross = torch.cdist(ir_node, rgb_node)    # [B,N,N]  IR->RGB

        # 3) intra mask（候选集合）
        M_ir = (D_ir <= self.thr_ir).to(ir_node.dtype)     # [B,N,N]
        M_rgb = (D_rgb <= self.thr_rgb).to(rgb_node.dtype) # [B,N,N]

        # 4) RBF 相似度（0~1），并乘 intra mask（保证超阈值为0）
        S_ir = torch.exp(-(D_ir ** 2) / (2.0 * self.sigma_ir * self.sigma_ir + 1e-8)) * M_ir
        S_rgb = torch.exp(-(D_rgb ** 2) / (2.0 * self.sigma_rgb * self.sigma_rgb + 1e-8)) * M_rgb

        # ### 跨模态相似度：不做 M_cross，只让 RBF 自然衰减
        S_cross = torch.exp(-(D_cross ** 2) / (2.0 * self.sigma_cross * self.sigma_cross + 1e-8))  # [B,N,N]

        # 5) 权重块
        W_ir_ir = S_ir                                 # [B,N,N]
        W_rgb_rgb = S_rgb                              # [B,N,N]

        # ### 关键：跨模态权重由“自己喜好（S_cross）”决定，但候选由对方模态内 mask 提供
        W_ir_rgb = S_cross * M_rgb                     # [B,N,N]  IR <- RGB
        W_rgb_ir = S_cross.transpose(1, 2) * M_ir      # [B,N,N]  RGB <- IR

        # 6) 组装 2N×2N
        H_top = torch.cat([W_ir_ir, W_ir_rgb], dim=2)   # [B,N,2N]
        H_bot = torch.cat([W_rgb_ir, W_rgb_rgb], dim=2) # [B,N,2N]
        H_2N2N = torch.cat([H_top, H_bot], dim=1)       # [B,2N,2N]

        # 7) 超图传播
        out_all = self.hgconv(ir_node, rgb_node, H_2N2N)  # [B,2N,C2]
        out_ir = out_all[:, :N, :]   # [B,N,C2]
        out_rgb = out_all[:, N:, :]  # [B,N,C2]

        # 8) 残差
        if self.residual:
            if self.res_proj is not None:
                out_ir = out_ir + self.res_proj(ir_node)
                out_rgb = out_rgb + self.res_proj(rgb_node)
            else:
                out_ir = out_ir + ir_node
                out_rgb = out_rgb + rgb_node

        # 9) reshape + 融合
        out_ir_map = out_ir.transpose(1, 2).contiguous().view(B, -1, H, W)
        out_rgb_map = out_rgb.transpose(1, 2).contiguous().view(B, -1, H, W)

        z = self.fuse(torch.cat([out_ir_map, out_rgb_map], dim=1))
