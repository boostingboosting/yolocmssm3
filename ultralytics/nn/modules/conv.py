...
__all__ = (
    ...
    "RepConv",
    "Index",
    'Silence', 'SilenceChannel', 'SilenceList', 'ChannelToNumber', 'NumberToChannel',
...
class SilenceList(nn.Module):
    def __init__(self,c2, index):
        super(SilenceList, self).__init__()
        self.index=index
    def forward(self, x):
        # print("len(x)",len(x))
        # print("len(x):", len(x))
        ...
        out = x[self.index]
        # print("SilenceListout.shape",out.shape)
        return out
