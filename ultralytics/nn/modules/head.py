...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, ch=()):
        ...
        self.nl = len(ch)  # number of detection layers ##检测层数
        # print("self.nl:", self.nl)
        if self.nl==6:
            self.nl = self.nl - 3
        # if self.nl==6 or self.nl==7:
        #     self.nl-=3
        # if self.nl==9 or self.nl==10:
        #     self.nl-=6
        # self.nl = len(ch)-3  # number of detection layers ##检测层数
        # self.nl = len(ch)-6  # number of detection layers ##检测层数
    ...
    def forward(self, x): #x为三层特征图+2个位置偏移量预测+2rgb目标中心位置预测
        ...
        if self.end2end:
            return self.forward_end2end(x[:self.nl])+x[self.nl:]

        for i in range(self.nl):#三层预测头
            v2 = self.cv2[i](x[i])
            v3 = self.cv3[i](x[i])
            x[i] = torch.cat((v2, v3), 1)
            # print("DetectV2",v2.shape)
            # print("DetectV3",v3.shape)
            # print("Detectx[i]",x[i].shape)
        if self.training:  # Training path
            return x
        y = self._inference(x[:self.nl])
