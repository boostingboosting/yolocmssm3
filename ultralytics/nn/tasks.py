...
from ultralytics.nn.modules import (
    ...
    HGStem,
    ImagePoolingAttn,
    Index, Silence, SilenceChannel, SilenceList, ChannelToNumber, NumberToChannel, DiverseBranchBlock, WideDiverseBranchBlock,
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, (DETECT_CLASS + SEGMENT_CLASS + POSE_CLASS + OBB_CLASS + V10_DETECT_CLASS)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            ...
            try:
                m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s))[:3]])  # forward on CPU
            except RuntimeError:
                try:
                    self.model.to(torch.device('cuda'))
                    m.stride = torch.tensor([s / x.shape[-2] for x in _forward(
                        torch.zeros(1, ch, s, s).to(torch.device('cuda')))[:3]])  # forward on CUDA
